...
namespace RBPhys
{
    ...
    {
        const float SOLVER_LINEAR_BETA = 0.5f;
        const float SOLVER_ANGULAR_BETA = 0.5f;
        ...
        void SolverInit(float dt, bool syncInit)
        {
            ...
            Vector3 ws_rb_pair_contact = pairRigidbody.Position + pairRigidbody.Rotation * local_rb_pair_contact;
            Quaternion ws_rb_contct_rot = rigidbody.Rotation * local_rb_contact_rot;
            Quaternion ws_rb_pair_contct_rot = pairRigidbody.Rotation * local_rb_pair_contact_rot;
        ...
        struct Jacobian
        {
            ...
            {
                Vector3 rA = contactPoint_a - rb_a.CenterOfGravityWorld;
                Vector3 rB = contactPoint_b - rb_b.CenterOfGravityWorld;
            ...
            internal (Vector3 v_a, Vector3 av_a, Vector3 v_b, Vector3 av_b) Resolve(RBRigidbody rb_a, RBRigidbody rb_b, Vector3 v_a, Vector3 av_a, Vector3 v_b, Vector3 av_b)
            {
                ...
                float angularLambda = (1 / (Vector3.Dot(_va, Vector3.Scale(rb_a.InverseInertiaWs, _va)) + Vector3.Dot(_vb, Vector3.Scale(rb_b.InverseInertiaWs, _vb)))) * -(jv_ang + _angularBias);
